 ...
 *   entirety and paste it here, changing the prefix from theme_ to simpleWhite_.
 *   For example:
 *
 *     original: theme_breadcrumb()
 *     theme override: simpleWhite_breadcrumb()
 *
 *   where simpleWhite is the name of your sub-theme. For example, the
 ...
 */
function simpleWhite_theme(&$existing, $type, $theme, $path) {
...
function simpleWhite_preprocess(&$vars, $hook) {
 ...
 */
/* -- Delete this line if you want to use this function
function simpleWhite_preprocess_page(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function simpleWhite_preprocess_node(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');

  // Optionally, run node-type-specific preprocess functions, like
  // simpleWhite_preprocess_node_page() or STARTERKIT_preprocess_node_story().
 ...
 */
/* -- Delete this line if you want to use this function
function simpleWhite_preprocess_comment(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function simpleWhite_preprocess_block(&$vars, $hook) {
  $vars['sample_variable'] = t('Lorem ipsum.');
}
// */

/* added by Sarah for simpleWhite */
...
*/
function simpleWhite_preprocess_block(&$vars, $hook) { //change mytheme to your theme name
  $block = $vars['block'];
  ...
  $cssid = simpleWhite_id_safe($block->title);
  }
  else  {
...
function simpleWhite_preprocess_node(&$vars) {
	...
	if ($vars['node']->taxonomy && $vars['type']=="blog")
	{
		...
		krsort($vocabulary, SORT_NUMERIC);
		unset($vars['terms']);
		$vars['terms'] = "";
		...
		foreach ($vocabulary as $vid => $terms) {
			...
			$vars['terms'] .= '<p>' . taxonomy_vocabulary_load($vid)->name . ": ";
			
			foreach ($terms as $term)
			{
				$vars['terms'] .= '<a href="/' . $term['href'] . '">' . $term['title'] . '</a>'; 
				
				$count++;

				if (sizeof($terms) > 1)
					$count <= sizeof($terms) ? $vars['terms'] .= ", " : $vars['terms'] .= " ";
